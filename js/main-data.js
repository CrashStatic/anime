const mainData = () => {
    ...
    const renderAnimeList = (array, ganres) => {
        ...
        ganres.forEach((ganre) => {
            ...
            productBlock.insertAdjacentHTML('beforeend', `
                <div class="row">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <div class="btn__all">
                            <a href="/anime/categories.html?ganre=${ganre}" class="primary-btn">View All <span class="arrow_right"></span></a>
                        </div>
                    </div>
                </div>
            `)
                ...
                listBlock.insertAdjacentHTML('beforeend', `
                    <div class="col-lg-4 col-md-6 col-sm-6">
                        <div class="product__item">
                            ...
                            ${tagsBlock.outerHTML}
                                <h5><a href="/anime/anime-details.html?itemId=${item.id}">${item.title}</a></h5>
                            </div>
                        </div>
                    </div>
                `)
            ...
            listBlock.querySelectorAll('.set-bg').forEach((elem) => {
        ...
        array.forEach((item) => {
            wrapper.insertAdjacentHTML('beforeend', `
                <div class="product__sidebar__view__item set-bg" data-setbg="${item.image}">
                    <div class="ep">${item.rating} / 10</div>
                    <div class="view"><i class="fa fa-eye"></i> ${item.views}</div>
                    <h5><a href="/anime/anime-details.html">${item.title}</a></h5>
                </div>
            `);
        });
